...
const ToastContainer = ({ toasts, removeToast }) => (
  <div className="fixed top-4 right-4 z-[200] flex flex-col gap-2 animate-in slide-in-from-right pointer-events-none">
    {toasts.map(toast => (
      <div key={toast.id} className={`pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg backdrop-blur-md border ${toast.type === 'success' ? 'bg-green-500/10 border-green-500/50 text-green-400' : toast.type === 'warning' ? 'bg-yellow-500/10 border-yellow-500/50 text-yellow-400' : 'bg-red-500/10 border-red-500/50 text-red-400'}`}>
    ...
    {(title || Icon) && (
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="flex items-center gap-2">
          {Icon && <div className="p-2 bg-slate-800 rounded-lg"><Icon className="text-cyan-400" size={18} /></div>}
          {title && <h3 className="text-white font-bold text-lg tracking-tight">{title}</h3>}
        </div>
      </div>
    )}
...
const SimpleChart = ({ data, color = "#22d3ee" }) => {
  if (!data || data.length < 2) return <div className="h-32 flex items-center justify-center text-slate-600 text-xs bg-slate-950/50 rounded-lg border border-dashed border-slate-800">Belum cukup data grafik</div>;
...
const InvestmentModal = ({ product, onClose, onConfirm }) => {
  ...
  const isHighRiskAmount = parseInt(amount) > 5000000; 
...
const App = () => {
  ...
  const handleWithdrawInvestment = async (inv) => {
    ...
    if (inv.risk === 'Tinggi') {
        if (luckFactor < 0.3) { actualReturnRate = -10; noteText = "⚠️ Rugi Investasi"; notify(`Investasi Rugi: ${formatRupiah(inv.amount * 0.1)}`, "error"); } 
        else if (luckFactor > 0.8) { actualReturnRate = inv.returnRate * 1.5; noteText = "🚀 Profit Tinggi"; notify("Cuan Tinggi!", "success"); }
    }
    ...
    if (isDemoMode) { setMyInvestments(prev => prev.filter(i => i.id !== inv.id)); } else { await deleteDoc(doc(db, "my_investments", inv.id)); }
  };
  ...
  const handleClaimLoan = async (loan) => {
    ...
    if (!isDemoMode) { await updateDoc(doc(db, "loans", loan.id), { status: 'Disbursed' }); } 
    else { setLoans(prev => prev.map(l => l.id === loan.id ? { ...l, status: 'Disbursed' } : l)); }
    await addTransactionFn({ amount: loan.amount, category: 'Pencairan Modal Pinjaman', type: 'income', note: `Pinjaman ${loan.reason} Disetujui`, paymentMethod: 'transfer', paymentProvider: 'Bank', transferStatus: 'Berhasil' });
              ...
              {showOtp ? (
                  <div className="animate-in fade-in zoom-in">
                      <div className="text-center mb-4">
                          <div className="bg-cyan-500/20 w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2"><KeyRound className="text-cyan-400" size={24}/></div>
                          <p className="text-slate-300 text-sm">Masukkan 4 digit kode yang telah dikirim.</p>
                      </div>
                      <div className="mb-4"><input type="text" maxLength="4" value={otpInput} onChange={(e) => setOtpInput(e.target.value.replace(/[^0-9]/g, ''))} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-3 text-white text-center text-2xl font-bold tracking-widest focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors" placeholder="0000" autoFocus /></div>
                      <Button type="submit" disabled={authLoading} className="w-full py-3 text-lg shadow-xl shadow-cyan-900/20">{authLoading ? 'Memverifikasi...' : 'Verifikasi OTP'}</Button>
                      <div className="mt-4 text-center"><button type="button" onClick={() => setShowOtp(false)} className="text-sm text-slate-500 hover:text-slate-300">Kembali</button></div>
                  </div>
              ) : (
                  <>
